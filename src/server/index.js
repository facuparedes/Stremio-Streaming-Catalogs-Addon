...
const ADMIN_TOKEN =
  process.env.ADMIN_TOKEN || process.env.VITE_ADMIN_TOKEN || null;

// Production error handling
if (process.env.NODE_ENV === "production") {
  const errorLog = fs.createWriteStream(
    path.join(__dirname, "../../vue/dist/error.log"),
  );
...
const tokenAuth = validateToken(ADMIN_TOKEN);
...
// Routes
app.get("/:configuration/manifest.json", tokenAuth, (req, res) => {
...
app.get("/manifest.json", (req, res) => {
  if (ADMIN_TOKEN) {
    return res
      .status(401)
      .json({ error: "Unauthorized: This addon requires a token" });
  }
